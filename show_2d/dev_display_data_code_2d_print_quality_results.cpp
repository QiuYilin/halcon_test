    ...
    HTuple hv_GenParamValue)
{
  ...
  //Available standards
  //
  //The quality standard parameter 'isoiec_tr_29158' was replaced by 'isoiec29158'
  //and is only available for backward compatibility.
  hv_SupportedQualityStandards.Clear();
  hv_SupportedQualityStandards[0] = "isoiec15415";
  hv_SupportedQualityStandards[1] = "isoiec29158";
  hv_SupportedQualityStandards[2] = "aimdpm_1_2006";
  hv_SupportedQualityStandards[3] = "isoiec_tr_29158";
  ...
  if (0 != (int((hv_SupportedModes.TupleFind(hv_Mode))==-1)))
  {
    throw HException("Unknown Mode: "+hv_Mode);
  }
  //
  //Check QualityStandard
  if (0 != (int((hv_SupportedQualityStandards.TupleFind(hv_QualityStandard))==-1)))
  {
  ...
  if (0 != (int(hv_Mode==hv_MODE_GRID)))
  {
    hv_DisplayCenter = 1;
    hv_DisplayCircle = 0;
    hv_DisplayLegend = 0;
    hv_MaxGrade = 4;
    if (0 != (int((hv_Color.TupleLength())==0)))
    {
      hv_Color = "gray";
    }
  }
  else if (0 != (int(hv_Mode==hv_MODE_BAD)))
  {
    hv_DisplayCenter = 0;
    hv_DisplayCircle = 1;
    hv_DisplayLegend = 0;
  ...
  else
  {
    ...
    {
    HTuple end_val62 = (hv_GenParamName.TupleLength())-1;
    HTuple step_val62 = 1;
    for (hv_I=0; hv_I.Continue(end_val62, step_val62); hv_I += step_val62)
    {
      //'center'
      if (0 != (int(HTuple(hv_GenParamName[hv_I])==hv_GEN_CENTER)))
      {
        //Check if values are valid
        if (0 != (HTuple(int(HTuple(hv_GenParamValue[hv_I])!=HTuple("true"))).TupleAnd(int(HTuple(hv_GenParamValue[hv_I])!=HTuple("false")))))
        {
        ...
        //Set new value
        hv_DisplayCenter = int(HTuple(hv_GenParamValue[hv_I])==HTuple("true"));
        //'circle'
      }
      else if (0 != (int(HTuple(hv_GenParamName[hv_I])==hv_GEN_CIRCLE)))
      {
        //Check if values are valid
        if (0 != (HTuple(int(HTuple(hv_GenParamValue[hv_I])!=HTuple("true"))).TupleAnd(int(HTuple(hv_GenParamValue[hv_I])!=HTuple("false")))))
        {
        ...
        //Set new value
        hv_DisplayCircle = int(HTuple(hv_GenParamValue[hv_I])==HTuple("true"));
      }
      else if (0 != (int(HTuple(hv_GenParamName[hv_I])==hv_GEN_LEGEND)))
      {
        //Check if values are valid
        if (0 != (HTuple(int(HTuple(hv_GenParamValue[hv_I])!=HTuple("true"))).TupleAnd(int(HTuple(hv_GenParamValue[hv_I])!=HTuple("false")))))
        {
        ...
        //Set new value
        hv_DisplayLegend = int(HTuple(hv_GenParamValue[hv_I])==HTuple("true"));
      }
      else if (0 != (int(HTuple(hv_GenParamName[hv_I])==hv_GEN_MAX_GRADE)))
      {
        //Check if values are valid
        if (0 != (HTuple(int(HTuple(hv_GenParamValue[hv_I])<0)).TupleOr(int(HTuple(hv_GenParamValue[hv_I])>4))))
        {
  ...
  while (0 != (int((hv_Color.TupleLength())<(hv_MaxGrade+1))))
  {
  ...
  {
  HTuple end_val128 = hv_MaxGrade;
  HTuple step_val128 = 1;
  for (hv_Grade=0; hv_Grade.Continue(end_val128, step_val128); hv_Grade += step_val128)
  {
    hv_GradeIdx = hv_ModuleGrades.TupleFind(hv_Grade);
    if (0 != (int(hv_GradeIdx<0)))
    {
      continue;
    }
