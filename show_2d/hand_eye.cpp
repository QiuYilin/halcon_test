...
// Chapter: Transformations / Misc
// Short Description: Determine the pose of the matched model in the base coordinate system. 
...
// Chapter: Transformations / Misc
// Short Description: Determine the pose of the matched model in the base coordinate system in a stationary camera setup. 
...
// Chapter: Calibration / Hand-Eye
// Short Description: Prepare the model to match and grasp. 
...
// Chapter: Calibration / Hand-Eye
// Short Description: Prepare the model to match and grasp in a stationary camera setup. 
    ...
    HTuple hv_ModelInBasePose, HTuple hv_Poses, HTupleVector/*{eTupleVector,Dim=1}*/ *hvec_ToolInBaseRobotPathPoses)
{
  ...
  HTuple  hv_Index1, hv_ToolInBaseRobotPathPose;

  {
  HTuple end_val0 = HTuple(hvec_ToolInModelRobotPathPoses.Length())-1;
  HTuple step_val0 = 1;
  for (hv_Index1=0; hv_Index1.Continue(end_val0, step_val0); hv_Index1 += step_val0)
  {
    PoseCompose(hv_ModelInBasePose, hvec_ToolInModelRobotPathPoses[hv_Index1].T(), 
        &hv_ToolInBaseRobotPathPose);
    ConvertPoseType(hv_ToolInBaseRobotPathPose, hv_Poses.TupleGetDictTuple("OrderOfTransform"), 
        hv_Poses.TupleGetDictTuple("OrderOfRotation"), hv_Poses.TupleGetDictTuple("ViewOfTransform"), 
        &hv_ToolInBaseRobotPathPose);
...
void calibrate_camera_and_plane_single_image (HTuple hv_CalibObjectData)
{
  ...
  hv_StartCamParam = hv_CalibObjectData.TupleGetDictTuple("StartCamParam");
  //
  //Check input
  if (0 != (HTuple(hv_StartCamParam[0]).TupleRegexpTest("line_scan")))
  {
  ...
  SetCalibDataCalibObject(hv_CalibDataID, 0, hv_CalibObjectData.TupleGetDictTuple("CalPlateDescr"));
  //Find the calibration plate.
  FindCalibObject(hv_CalibObjectData.TupleGetDictObject("ImageCaltab"), hv_CalibDataID, 
      0, 0, 0, HTuple(), HTuple());
  ...
  SetOriginPose(hv_PlaneInCamPose0, 0, 0, hv_CalibObjectData.TupleGetDictTuple("CalPlateThickness"), 
      &hv_PlaneInCamPose);
  ...
  SetDictTuple(hv_CalibObjectData, "ErrorCamCalibInPixel", hv_ErrorCamCalibInPixel);
  SetDictTuple(hv_CalibObjectData, "CamParam", hv_CamParam);
  SetDictTuple(hv_CalibObjectData, "PlaneInCamPose", hv_PlaneInCamPose);
  //Clean up.
    ...
    HTuple hv_CalibObjectData, HTuple *hv_HandEyeCalibData)
{
  ...
  //
  hv_CamParam = hv_CalibObjectData.TupleGetDictTuple("CamParam");
  hv_CalPlateThickness = hv_CalibObjectData.TupleGetDictTuple("CalPlateThickness");
  hv_PlaneInCamPose = hv_CalibObjectData.TupleGetDictTuple("PlaneInCamPose");
  //
  //Check input
  if (0 != (HTuple(HTuple(int((hv_RowsTouchingPointInPlane.TupleLength())<3)).TupleOr(int((hv_ColumnsTouchingPointInPlane.TupleLength())<3))).TupleOr(int(HTuple(hvec_ToolInBasePoses.Length())<3))))
  {
    throw HException("Please specify at least three image coordinates and robot poses.");
  }
  if (0 != (HTuple(int((hv_RowsTouchingPointInPlane.TupleLength())!=(hv_ColumnsTouchingPointInPlane.TupleLength()))).TupleOr(int((hv_RowsTouchingPointInPlane.TupleLength())!=HTuple(hvec_ToolInBasePoses.Length())))))
  {
    throw HException("The number of image coordinates and robot poses have to be equal.");
  }
  if (0 != (HTuple(hv_CamParam[0]).TupleRegexpTest("line_scan")))
  {
    throw HException("Line scan cameras are not supported");
  }
  //
  ...
  for (hv_Index=1; hv_Index.Continue(end_val21, step_val21); hv_Index += step_val21)
  {
    GetPoseType(hvec_ToolInBasePoses[hv_Index].T(), &hv_OrderOfTransform, &hv_OrderOfRotation, 
        &hv_ViewOfTransform);
    if (0 != (HTuple(HTuple(int(hv_OrderOfTransform0!=hv_OrderOfTransform)).TupleOr(int(hv_OrderOfRotation0!=hv_OrderOfRotation))).TupleOr(int(hv_ViewOfTransform0!=hv_ViewOfTransform))))
    {
  ...
  //
  //Create output dict.
  CreateDict(&(*hv_HandEyeCalibData));
  SetDictTuple((*hv_HandEyeCalibData), "CamParam", hv_CamParam);
  SetDictTuple((*hv_HandEyeCalibData), "BaseInCamPose", hv_BaseInCamPose);
  SetDictTuple((*hv_HandEyeCalibData), "PlaneInBasePose", hv_PlaneInBasePose);
  SetDictTuple((*hv_HandEyeCalibData), "PlaneInCamPose0", hv_PlaneInCamPose);
  SetDictTuple((*hv_HandEyeCalibData), "PlanePointsRMS", hv_PlanePointsRMS);
  SetDictTuple((*hv_HandEyeCalibData), "PlanePointsMaxDiff", hv_PlanePointsMaxDiff);
  return;
}
    ...
    HTuple hv_DistancePlaneToCamera, HTuple hv_Width, HTuple hv_Height, HTuple *hv_HandEyeCalibData)
{
  ...
  //Check input.
  if (0 != (HTuple(HTuple(int((hv_RowsTouchingPointInPlane.TupleLength())<4)).TupleOr(int((hv_ColumnsTouchingPointInPlane.TupleLength())<4))).TupleOr(int(HTuple(hvec_ToolInBasePoses.Length())<4))))
  {
  ...
  if (0 != (HTuple(int(hv_Width<=0)).TupleOr(int(hv_Height<=0))))
  {
    throw HException("Width or Height must be greater than 0.");
  }
  if (0 != (int(hv_DistancePlaneToCamera<=0)))
  {
  ...
  for (hv_NumFocus=0; hv_NumFocus.Continue(end_val67, step_val67); hv_NumFocus += step_val67)
  {
    ...
    if (0 != (int(hv_ErrorBasePoseInPixel>hv_ErrorBasePoseInPixelTmp)))
    {
  ...
  if (0 != (int(hv_CosAngleBetweenZAxis<0)))
  {
    ...
    HTuple hv_CalPlateThickness, HTuple hv_StartCamParam, HTuple *hv_CalibObjectData)
{
  ...
  //
  //Create output dict.
  CreateDict(&(*hv_CalibObjectData));
  SetDictObject(ho_ImageCaltab, (*hv_CalibObjectData), "ImageCaltab");
  SetDictTuple((*hv_CalibObjectData), "CalPlateDescr", hv_CalPlateDescr);
  SetDictTuple((*hv_CalibObjectData), "CalPlateThickness", hv_CalPlateThickness);
  SetDictTuple((*hv_CalibObjectData), "StartCamParam", hv_StartCamParam);
  SetDictTuple((*hv_CalibObjectData), "FinderRow", hv_FinderRow);
  SetDictTuple((*hv_CalibObjectData), "FinderColumn", hv_FinderColumn);
  SetDictTuple((*hv_CalibObjectData), "MarksPerRow", hv_MarksPerRow);
  return;
}
    ...
    HTuple *hv_OM3DTool, HTuple *hv_OM3DPlane)
{
  ...
  //current positions.
  //
  hv_ToolInCamPose = hv_HandEyeCalibData.TupleGetDictTuple("ToolInCamPose");
  hv_CamParam = hv_HandEyeCalibData.TupleGetDictTuple("CamParam");
  hv_PlaneInBasePose0 = hv_HandEyeCalibData.TupleGetDictTuple("PlaneInBasePose0");
  //
  if (0 != (int(hv_CameraSize<=0)))
  {
    ...
    HTuple *hv_OM3DPlane, HTuple *hv_OM3DBase, HTuple *hv_OM3DToolOrigin)
{
  ...
  // Local control variables
  HTuple  hv_CamParam, hv_PlaneInCamPose0, hv_CX;
  HTuple  hv_CY, hv_OptAxisPlaneX, hv_OptAxisPlaneY, hv_HomMat3D;
  HTuple  hv_OptAxisCamX, hv_OptAxisCamY, hv_OptAxisCamZ;
  ...
  //
  hv_CamParam = hv_HandEyeCalibData.TupleGetDictTuple("CamParam");
  hv_PlaneInCamPose0 = hv_HandEyeCalibData.TupleGetDictTuple("PlaneInCamPose0");
  //
  ...
  ClearCameraSetupModel(hv_CameraSetupModelID);
  hv_OM3DCameraOrigin = hv_OM3DCameraOrigin.TupleConcat(hv_OM3DConeOrig);
  PoseInvert(hv_HandEyeCalibData.TupleGetDictTuple("BaseInCamPose"), &hv_CamInBasePose);
  RigidTransObjectModel3d(hv_OM3DCameraOrigin, hv_CamInBasePose, &(*hv_OM3DCamera));
  ClearObjectModel3d(hv_OM3DCameraOrigin);
  //
  //Create 3D object model of plane.
    ...
    HTuple hv_HandEyeCalibData, HTuple hv_RectificationData, HTuple *hv_OM3DModel)
{
  ...
  //
  hv_CamParam = hv_HandEyeCalibData.TupleGetDictTuple("CamParam");
  hv_MatchingPlaneInCamPose = hv_Poses.TupleGetDictTuple("MatchingPlaneInCamPose");
  hv_RectifyImage = hv_RectificationData.TupleGetDictTuple("RectifyImage");
  if (0 != (HTuple(HTuple(int(hv_RectifyImage==HTuple("true"))).TupleOr(int(hv_RectifyImage==HTuple("only_rectify")))).TupleOr(int(hv_RectifyImage==HTuple("align_and_rectify")))))
  {
    hv_ScaleRectification = hv_RectificationData.TupleGetDictTuple("ScaleRectification");
  }
  //
    ...
    HTuple hv_RobotTouchingPointInToolCoordinates, HTuple *hv_OM3DToolTouchingPoint)
{
  ...
  // Local control variables
  HTuple  hv_OM3DToolOrigin, hv_OM3DBase, hv_OM3DToolTouchingPointTmp;
  HTuple  hv_Index, hv_OM3DRigidTrans;
    ...
    HTuple *hv_ColumnFinderPattern)
{
  ...
  //
  hv_CamParam = hv_CalibObjectData.TupleGetDictTuple("CamParam");
  hv_CalPlateDescr = hv_CalibObjectData.TupleGetDictTuple("CalPlateDescr");
  hv_MarksPerRow = hv_CalibObjectData.TupleGetDictTuple("MarksPerRow");
  hv_FinderRow = hv_CalibObjectData.TupleGetDictTuple("FinderRow");
  hv_FinderColumn = hv_CalibObjectData.TupleGetDictTuple("FinderColumn");
  //
  //Check input.
  //
  //Check image coordinates.
  if (0 != (HTuple(int((hv_RowNearFinderPattern.TupleLength())>1)).TupleOr(int((hv_ColumNearFinderPattern.TupleLength())>1))))
  {
    throw HException("Please specify only one image coordinate.");
  }
  //Check number of marks per row.
  if (0 != (int(hv_MarksPerRow<3)))
  {
    ...
    HTuple *hv_RobotTouchingPointInToolCoordinates)
{
  ...
  //Check input.
  if (0 != (int(HTuple(hvec_ToolInBasePosesTouchingPoint.Length())<3)))
  {
  ...
  for (hv_Index=1; hv_Index.Continue(end_val33, step_val33); hv_Index += step_val33)
  {
    ...
    //Get rotation axis relative to first pose.
    {
  ...
  if (0 != hv_DetailedErrors)
  {
    ...
    {
      if (0 != (int(hvec_DiffToIdentity[hv_Index].T()<hv_MinDiffToIdentity)))
      {
    ...
    for (hv_Index=1; hv_Index.Continue(end_val67, step_val67); hv_Index += step_val67)
    {
      ...
      for (hv_Index1=hv_Index+1; hv_Index1.Continue(end_val68, step_val68); hv_Index1 += step_val68)
      {
        ...
        if (0 != (int(hv_CosAngle<hv_MinCosAngle)))
        {
  ...
  if (0 != (int(hv_MinSingularValue<0.15)))
  {
    ...
    HTuple *hv_ModelInBasePose)
{
  ...
  hv_CamParam = hv_HandEyeCalibData.TupleGetDictTuple("CamParam");
  hv_ToolInCamPose = hv_HandEyeCalibData.TupleGetDictTuple("ToolInCamPose");
  hv_PlaneInModelPose = hv_Poses.TupleGetDictTuple("PlaneInModelPose");
  hv_RectifyImage = hv_RectificationData.TupleGetDictTuple("RectifyImage");
  if (0 != (int(hv_RectifyImage!=HTuple("no_rectification"))))
  {
    hv_ScaleRectification = hv_RectificationData.TupleGetDictTuple("ScaleRectification");
  }
  hv_MatchingPlaneRectifiedPartInCamPose = hv_RectificationData.TupleGetDictTuple("MatchingPlaneRectifiedPartInCamPose");
  //
  //Keep track of the pose type used by the robot.
  ...
  if (0 != (int((hv_Row.TupleLength())==1)))
  {
    ...
    else if (0 != (HTuple(int(hv_RectifyImage==HTuple("only_rectify"))).TupleOr(int(hv_RectifyImage==HTuple("align_and_rectify")))))
    {
      hv_HomMat3DObject.Clear();
      hv_HomMat3DObject.Append(HTuple(hv_HomMat2DObject[4]));
      hv_HomMat3DObject.Append(HTuple(hv_HomMat2DObject[3]));
    ...
    HTuple hv_HandEyeCalibData, HTuple hv_Poses, HTuple hv_RectificationData, HTuple *hv_ModelInBasePose)
{
  ...
  //the robot.
  hv_CamParam = hv_HandEyeCalibData.TupleGetDictTuple("CamParam");
  hv_BaseInCamPose = hv_HandEyeCalibData.TupleGetDictTuple("BaseInCamPose");
  hv_PlaneInModelPose = hv_Poses.TupleGetDictTuple("PlaneInModelPose");
  hv_MatchingPlaneInCamPose = hv_Poses.TupleGetDictTuple("MatchingPlaneInCamPose");
  hv_RectifyImage = hv_RectificationData.TupleGetDictTuple("RectifyImage");
  if (0 != (int(hv_RectifyImage==HTuple("true"))))
  {
    hv_ScaleRectification = hv_RectificationData.TupleGetDictTuple("ScaleRectification");
  }
  //
  ...
  ConvertPoseType(hv_PlaneInModelPose, "Rp+T", "gba", "point", &hv_PlaneInModelPose);
  if (0 != (HTuple(HTuple(int((hv_Row.TupleLength())==1)).TupleAnd(int((hv_Column.TupleLength())==1))).TupleAnd(int((hv_Angle.TupleLength())==1))))
  {
    VectorAngleToRigid(0, 0, 0, hv_Row, hv_Column, hv_Angle, &hv_HomMat2DObject);
    //col = x, row = y
    if (0 != (int(hv_RectifyImage==HTuple("false"))))
    {
      AffineTransPixel(hv_HomMat2DObject, 0, 0, &hv_RowObject, &hv_ColObject);
    ...
    else if (0 != (int(hv_RectifyImage==HTuple("true"))))
    {
      hv_HomMat3DObject.Clear();
      hv_HomMat3DObject.Append(HTuple(hv_HomMat2DObject[4]));
      hv_HomMat3DObject.Append(HTuple(hv_HomMat2DObject[3]));
    ...
    HTuple hv_RectifyImage, HTuple hv_HandEyeCalibData, HTuple *hv_Poses, HTuple *hv_RectificationData)
{
  ...
  HTuple  hv_ModelInPlanePose, hv_PlaneInModelPose;
  ...
  CreateDict(&(*hv_Poses));
  SetDictTuple((*hv_Poses), "PlaneInCamPose", hv_PlaneInCamPose);
  SetDictTuple((*hv_Poses), "CamInBasePose", hv_CamInBasePose);
  SetDictTuple((*hv_Poses), "PlaneInBasePose", hv_PlaneInBasePose);
  SetDictTuple((*hv_Poses), "MatchingPlaneInCamPose", hv_MatchingPlaneInCamPose);
  SetDictTuple((*hv_Poses), "MatchingPlaneInBasePose", hv_MatchingPlaneInBasePose);
  SetDictTuple((*hv_Poses), "PlaneInModelPose", hv_PlaneInModelPose);
  //
  //Create dict for rectification data.
  CreateDict(&(*hv_RectificationData));
  SetDictTuple((*hv_RectificationData), "RectifyImage", hv_RectifyImage);
  if (0 != (int(hv_RectifyImage!=HTuple("no_rectification"))))
  {
    SetDictTuple((*hv_RectificationData), "ScaleRectification", hv_ScaleRectification);
  }
  SetDictTuple((*hv_RectificationData), "MatchingPlaneRectifiedPartInCamPose", hv_MatchingPlaneRectifiedPartInCamPose);
  return;
}
    ...
    HTuple hv_RectifyImage, HTuple hv_HandEyeCalibData, HTuple *hv_Poses, HTuple *hv_RectificationData)
{
  ...
  {
    SetDictTuple((*hv_RectificationData), "ScaleRectification", hv_ScaleRectification);
    SetDictObject(ho_RectificationMap, (*hv_RectificationData), "RectificationMap");
  }
  return;
}
    ...
    HTuple hv_ToolInBasePose, HTuple hv_HandEyeCalibData, HTuple hv_Poses, HTuple hv_RectificationData)
{
  ...
  // Local control variables
  HTuple  hv_CamParam, hv_ToolInCamPose, hv_RectifyImage;
  HTuple  hv_ScaleRectification, hv_MatchingPlaneInBasePose;
  HTuple  hv_OrderOfTransform, hv_OrderOfRotation, hv_ViewOfTransform;
  HTuple  hv_BaseInToolPose, hv_BaseInCamPose, hv_MatchingPlaneInCamPose;
  HTuple  hv_MatchingPlaneRectifiedPartInCamPose, hv_Width;
  HTuple  hv_Height, hv_ClipRegion, hv_BorderRows, hv_BorderColumns;
  HTuple  hv_BorderX, hv_BorderY, hv_MatchingPlaneRectifiedPartInMatchingPlanePose;
  HTuple  hv_WidthRect, hv_HeightRect;
  ...
  hv_ToolInCamPose = hv_HandEyeCalibData.TupleGetDictTuple("ToolInCamPose");
  hv_RectifyImage = hv_RectificationData.TupleGetDictTuple("RectifyImage");
  if (0 != (HTuple(int(hv_RectifyImage==HTuple("only_rectify"))).TupleOr(int(hv_RectifyImage==HTuple("align_and_rectify")))))
  {
    hv_ScaleRectification = hv_RectificationData.TupleGetDictTuple("ScaleRectification");
  }
  hv_MatchingPlaneInBasePose = hv_Poses.TupleGetDictTuple("MatchingPlaneInBasePose");
  //
  //Keep track of the pose type used by the robot.
  GetPoseType(hv_ToolInBasePose, &hv_OrderOfTransform, &hv_OrderOfRotation, &hv_ViewOfTransform);
  //Convert to default pose type.
  ...
  {
    if (0 != (int(hv_RectifyImage==HTuple("only_rectify"))))
    {
